...
describe('PaginationSSR 컴포넌트', () => {
  it('총 페이지가 1 이하일 때는 렌더링되지 않아야 한다', () => {
    const { container } = render(<CustomPagination currentPage={1} totalPages={1} />);

    expect(container.firstChild).toBeNull();
  });

  it('현재 페이지가 활성화되어야 한다', () => {
    render(<CustomPagination currentPage={2} totalPages={3} />);
  ...
  it('페이지 링크가 올바른 URL을 가져야 한다', () => {
    render(<CustomPagination currentPage={1} totalPages={3} />);
  ...
  it('마지막 페이지에서 다음 버튼이 비활성화되어야 한다', () => {
    render(<CustomPagination currentPage={3} totalPages={3} />);
