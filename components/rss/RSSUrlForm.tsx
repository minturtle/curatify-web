...
export default function RSSUrlForm() {
  const [state, formAction, pending] = useActionState<AddRSSUrlActionResult, FormData>(
    addRSSUrlAction,
    null
  );
  const [rssType, setRssType] = useState<RSSType>('normal');
  ...
  return (
    <form
      action={formAction}
      className="w-full max-w-md md:max-w-2xl lg:max-w-4xl mx-auto space-y-4 flex gap-2"
    >
      <Select
        name="type"
        value={rssType}
      ...
      >
        ...
        <SelectContent>
          <SelectItem value="normal">일반 RSS</SelectItem>
